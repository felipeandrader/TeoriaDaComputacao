📊 Comparação do Algoritmo Heap Sort em C e Python

📌 Sobre o Projeto

Este repositório contém um estudo sobre o desempenho do algoritmo Heap Sort nas linguagens C e Python. O objetivo é comparar o comportamento e eficiência das duas implementações em diferentes cenários de execução, analisando o impacto da linguagem no tempo total de processamento:

✅ Melhor caso
⚠️ Caso médio
❌ Pior caso

A proposta é observar como cada linguagem se sai com o mesmo algoritmo, destacando vantagens e desvantagens de cada implementação em termos de performance e otimização.

💻 Linguagens Utilizadas

* C
* Python 3

📈 Objetivos da Análise

* Avaliar o desempenho do Heap Sort em diferentes linguagens de programação;
* Comparar os tempos de execução entre C e Python;
* Medir o tempo de execução nos seguintes cenários:

  * Melhor caso: vetor já ordenado;
  * Pior caso: vetor em ordem inversa;
  * Caso médio: vetor com elementos em ordem aleatória.

🧠 O que é o Heap Sort?

O Heap Sort é um algoritmo de ordenação baseado em estrutura de dados heap, geralmente um heap máximo. Ele organiza os elementos de modo que o maior elemento fique sempre na raiz, facilitando sua remoção e ordenação. Suas complexidades são:

* Melhor caso: O(n log n)
* Caso médio: O(n log n)
* Pior caso: O(n log n)

📁 Organização do Repositório

* Pasta C → implementação dos algorítmos Heap Sort em C
* Pasta python → implementação dos algorítmos Heap Sort em Python
* `README.md` → este arquivo com as informações do projeto
